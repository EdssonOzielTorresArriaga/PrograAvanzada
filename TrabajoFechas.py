# Un colegio tiene programado un pago para el dia 15 de febrero de 2020. Si el pago es realizado despues de la fecha, se cobran $8 pesos por cada dia transcurrido. El pago es por $5,500.00.
...
def main():
    ...
    cargodiario=8.00
    # Cuando hacemos una operacion con fechas, se obtiene lo que se conoce como "datetime delta" que es un periodo de tiempo como podria ser x minutos, x horas o dias. NO es un numero. En este caso diferencia es el datetime delta que indica la diferencia en tiempo de la fecha compromiso y la fecha real
    diferencia=fechapago-fechacomprimiso
    pagofinal=0.00
    # Si la diferencia en tiempo, expresada en terminos de dias, es mayor a 0, quiere decir que se pago fuera de tiempo y hay recargo.
